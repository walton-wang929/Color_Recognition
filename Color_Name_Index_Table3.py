...
def _round_rgb_16x(rgb_triplet, x = 1):
    rgb = tuple([_round_16x(n,x) for n in rgb_triplet])
    return rgb
...
def _round_rgb_16(rgb_triplet):
    return _round_rgb_16x(rgb_triplet)
...
def get_dominant_color(flat_img, num_pixels = 3, coarse_mode = True):
    if coarse_mode:
        x = 2
    else:
        x = 1

    flat_img = [_round_rgb_16x(pixel,x) for pixel in flat_img]
    hist, counts = np.unique(flat_img, axis = 0, return_counts = True)

    num_pixels = len(hist) if len(hist) < num_pixels else num_pixels
